...
10. Confirma la hipñótesis de que las salidas anticipadas de los vuelos en los minutos 20-30 y 50-60 son causadas por vuelos programados que salen temprano. Sugerencia: crea una variable binaria que te diga si un vuelo se retrasó o no.
...
Vamos a crear una función que contenga la Lada, ya que existen muchas, se pueden consultar, para México, en la siguiente [liga](http://www.bajarealestategroup.net/pdf/mexican_area_codes.pdf).
...
Comienzan con tres consonantes :\b[^AEIOUaeiou\s]{3}
```{r}
...
Se considera una palabra a una sucesión de caracteres (al menos dos)
Comienzan y terminan con el mismo caracter:

Con la expresión regular \b([A-z])\S+\1\b se obtienen aquellas palabras que comienzan y terminan con el mismo caracter 
```{r}
...
Contienen una letra repetida en al menos tres lugares (por ejemplo, "eleven" contiene tres "e")
Regex : \b[A-z]*([A-z])[A-z]*\1[A-z]*\1[A-z]*\b
...
20. Encuentra todas las contracciones. Separa las piezas antes y después del apóstrofe.
...
21. Reemplaza todas las barras diagonales en una cadena con barras diagonales inversas.
...
22. Implementa una versión simple de str_to_lower() usando replace_all().
...
23. Cambia la primera y la última letra en words. ¿Cuáles de esas cadenas son todavía palabras?
...
24. Divide una cadena como “apples, pears, and bananas” en componentes individuales.

```{r}
str_split("apples, pears, and bananas", "(, | )")
```

25. ¿Por qué es mejor dividir con boundary("word") que con " "?
...
26. ¿Qué hace la división con una cadena vacía ("")? Experimenta y luego lee la documentación.
...
27. ¿Cuáles son las cinco palabras más comunes en sentences?
...
28. Encuentra las funciones de stringi que:
...
[Documentación de Stringi](https://cran.r-project.org/web/packages/stringi/stringi.pdf)

29. ¿Cómo controlas el lenguaje que stri_sort() usa para ordenar?
