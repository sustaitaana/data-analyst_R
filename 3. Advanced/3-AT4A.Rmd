...
paste0() Según la documentación es un poco más eficiente que paste()
También el argumento sep en paste() es " " y en paste0() es "", es decir paste() introduce un espacio por default.
Es equivalente a "str_c()"

2. En tus propias palabras, describe la diferencia entre los argumentos `sep` y `collapse` de `str_c()`.
sep* inserta un caracter entre cada palabra concatenada, en cada concatenación
collapse* inserta una cadena después de cada iteración de concatenación, por ejemplo si tenemos varios vectores.
...
```{r}
de_enmedio <- function(cadena){
  longitud <- str_length(cadena)
  str_c("longitud de cadena: ", longitud)
  mitad <- longitud/2
  str_c("Letra de en medio: ",str_sub(cadena, mitad, mitad))
}
cadenita <- "cadena para separar"
...
concatenador <- function(arreglo){
  ...
  if(longitud>0){
    ...
    final <- str_replace(final, ", $", " y ")
    print(final)
    final <- str_c(final, arreglo[longitud])
    print(final)
  }
...
11. Dado el corpus de palabras en `stringr::words`, crea expresiones regulares que encuentren todas las palabras que:
    ...
    ```{r}
    wordsi <- stringr::words
    palabras <- str_extract(words, "^y", simplify=TRUE)
    head(palabras)
    ```
...
12. Crea expresiones regulares para encontrar todas las palabras que:
    ...
    /\b[aeiou]\w*/
    R. Se empieza por un delimitador boundary \b y luego la opción de que empiece la expresión con una letra del abecedario [aeiou] y al final que se termine la palabra con w*
...
Con lada
```{r, message=FALSE, warning=FALSE}
str_view("teléfono con lada: 5551211350, otro teléfono 123123", "\\b[0123456789]{10}\\b")
```
...
15. Crea expresiones regulares para encontrar todas las palabras que:
    ...
    ```{r}
    str_view_all("MMMOLA IMAGINE LLLAOM LL0M lkmh ghj kjasdn76", "(\\b[^AEIOUaeiou\\s]{3})")
    ```
...
```
Sales NA, porque no pudo hacer el parsing

2. ¿Qué hace el argumento `tzone` para `today()`? ¿Por qué es importante?
R. Para obtener la hora específica en un cierto lugar del globo
...
Se debería usar dept_time, porque fue el tiempo real al cual se fue, no el planeado.
...
miEdad <- function(dateIn){
  todayDate <- today()
  diffYears <- year(todayDate) - year(dateIn)
  if(month(todayDate) >= month(dateIn)){
    if(day(dateIn) < day(todayDate)){
      print(str_c("Tienes: ", diffYears-1, " años"))
    }
    else{
      print(str_c("Tienes: ", diffYears, " años"))
    }
  }
  else{
    print(str_c("Tienes: ", diffYears-1, " años"))
