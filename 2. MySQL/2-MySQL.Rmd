...
18. Calcula el retraso promedio por destino, luego une con la tabla `airports` para que puedas mostrar la distribución espacial de los retrasos. Aquí hay una manera fácil de dibujar un mapa de los Estados Unidos:
...
19. Agrega la ubicación del origen y el destino (es decir, `lat` y `lon`) a `flights`.

20. ¿Qué pasó el 13 de junio de 2013?

¿Qué significa para un vuelo tener una falta tailnum? ¿Qué planestienen en común los números de cola que no tienen un registro coincidente ? (Sugerencia: una variable explica ~ 90% de los problemas).

Filtra vuelos para mostrar solo vuelos con aviones que hayan volado al menos 100 vuelos.

Combine fueleconomy::vehiclesy fueleconomy::commonencuentre solo los registros de los modelos más comunes.

Encuentra las 48 horas (en el transcurso del año) que tienen las peores demoras. Haga una referencia cruzada con los weatherdatos. ¿Puedes ver algún patrón?

¿Qué anti_join(flights, airports, by = c("dest" = "faa"))te dice? ¿Qué anti_join(airports, flights, by = c("faa" = "dest"))te dice?

Es de esperar que exista una relación implícita entre el avión y la línea aérea, ya que cada avión es volado por una sola línea aérea. Confirme o rechace esta hipótesis usando las herramientas que aprendió anteriormente.
