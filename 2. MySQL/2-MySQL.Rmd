...
Una **función de ventana** es una variación de una función de agregación. Cuando una función de agregación, como `sum()` y `mean()`, toma $n$ entradas y devuelve un único valor, una función de ventana devuelve $n$ valores. La salida de una función de ventana depende de todos sus valores de entrada, por lo que no incluyen funciones que sean utilizadas elemento a elemento, como `+` o `round()`. Las funciones de ventana incluyen variaciones en las funciones de agregación, como `cumsum()` y `cummean()`, funciones para clasificar y ordenar como `rank()`, y funciones para encontrar contrapesos, como `lead()` y `lag()`.

Usaremos una pequeña muestra del conjunto de datos de bateo de `Lahman`, que contiene a los jugadores que han ganado un premio.
...
+ Por cada jugador, encuentra los dos años con más hits

```{r, message=FALSE, warning=FALSE}
filter(players, min_rank(desc(H)) <= 2 & H > 0)
```

+ Para cada jugador, clasifica cada año por la cantidad de juegos jugados

```{r, message=FALSE, warning=FALSE}
mutate(players, G_rank = min_rank(G))
```

+ Para cada jugador, calcula el cambio promedio en los juegos jugados por año
...
Hay cinco familias principales de funciones de ventana. Dos de las cuales no están relacionadas con las funciones de agregación:

+ Funciones de clasificación y ordenamiento: `row_number()`, `min_rank()`, `dense_rank()`, `cume_dist()`, `percent_rank()`, y `ntile()`. Todas estas funciones toman un vector para ordenar y devuelven varios tipos de rangos.

+ Contrapesos `lead()` y `lag()` permiten acceder a los valores anteriores y siguientes en un vector, lo que facilita el cálculo de diferencias y tendencias.

Las otras tres familias son variaciones en las funciones de agregación que ya conocemos:

+ Agregaciones acumulativas: `cumsum()`, `cummin()`, `cummax()` (de la base R), y `cumall()`, `cumany()` y `cummean()` (de `dplyr`).

+ Las agregaciones móviles operan en una ventana de ancho fijo. Estos no son parte de R base ni de `dplyr`, pero hay muchas implementaciones en otros paquetes, como [RcppRoll](https://cran.r-project.org/web/packages/RcppRoll/index.html).
  
+ Agregaciones recicladas, donde se repite una agregación para que coincida con la longitud de la entrada. Estos no son necesarios en R porque el reciclaje de vectores recicla las agregaciones automáticamente cuando es necesario. Son importantes en SQL, porque la presencia de una función de agregación generalmente le dice a la base de datos que devuelva solo una fila por grupo.

#### rank

Las funciones de clasificación son variaciones de un mismo tema, sólo cambia cómo se manejan las ligas a los datos:
...
Otras dos funciones de clasificación devuelven números entre 0 y 1. `percent_rank()` da el porcentaje del rango mientras que `cume_dist()` da la proporción de valores menores o iguales que el valor actual.
...
Estos son útiles si deseamos seleccionar (por ejemplo) el 10% superior de registros dentro de cada grupo. Por ejemplo:

```{r, message=FALSE, warning=FALSE}
filter(players, cume_dist(desc(G)) < 0.1)
```

Finalmente, `ntile()` divide los datos en $n$-cubos de tamaño uniforme. Es una clasificación aproximada, y se puede utilizar `mutate()` para dividir los datos en segmentos para un resumen posterior. 

Por ejemplo, podríamos usar `ntile()` para dividir a los jugadores dentro de un equipo en cuatro grupos clasificados y calcular el número promedio de juegos dentro de cada grupo.
...
Todas las funciones de clasificación se clasifican de menor a mayor, por lo que los valores de entrada pequeños obtienen rangos pequeños. Debemos usar `desc()` para clasificar de mayor a menor.

#### lead & lag

`lead()` y `lag()` producen versiones "movidas" de un vector de entrada que está por delante o por detrás del vector original.
...
El uso `lag()` es más conveniente que `diff()` ya que para las $n$ entradas `diff()` devuelve $n - 1$ salidas.

+ Averiguar cuándo cambia un valor.

```{r, message=FALSE, warning=FALSE}
filter(players, teamID != lag(teamID))
```

`lead()` y `lag()` tienen un argumento opcional `order_by`. Si se establece, en lugar de usar el orden de fila para determinar qué valor viene antes que otro, usarán otra variable. Esto es importante si no tenemos ordenados los datos, o si deseamos ordenarlos de una manera y retrasarlos.

Aquí un pequeño ejemplo de lo que sucede si no especificamos `order_by` cuando es necesario:
...
En R base tenemos algunas funciones acumultavias como la suma (`cumsum()`), el mínimo (`cummin()`) y el máximo (`cummax()`). Otras funciones de acumulación comunes son `cumany()` y `cumall()`, versiones acumulativas de `||` y `&&`, y `cummean()`, una media acumulativa. Estas pertenecen a `dplyr`.

`cumany()` y `cumall()` son útiles para seleccionar todas las filas hasta, o todas las filas posteriores a una condición que es verdadera la primera (o última) vez. Por ejemplo, podemos usar `cumany()` para encontrar todos los registros de un jugador después de que haya jugado un año con 150 juegos:

```{r, message=FALSE, warning=FALSE}
filter(players, cumany(G > 150))
```

Al igual que con lead y lag, es posible que deseemos controlar el orden en que se produce la acumulación. Ninguna de las funciones integradas tienen un argumento `order_by` argumento por lo que `dplyr` nos ofrece un ayudante: la función `order_by()`:
...
Esta función usa evaluación no estándar, por lo que no es recomendable usarla dentro de otra función, en su lugar se recomienda usar el más simple pero menos conciso `with_order()`.

#### Agregaciones recicladas

El reciclaje de vector de R facilita la selección de valores que son más altos o más bajos que un resumen. Se llaman agregaciones recicladas porque el valor del agregado se recicla para que tenga la misma longitud que el vector original. Son útiles si estamos buscando todos los registros mayores que la media o menores que la mediana:
...
Si bien la mayoría de las bases de datos SQL no tienen un equivalente de `median()` o `quantile()`, al filtrar podemos lograr el mismo efecto con `ntile()`. 
...
También podemos usar esta idea para seleccionar los registros con el valor más alto (`x == max(x)`) o más bajo (`x == min(x)`) para un campo, pero las funciones de clasificación nos brindan más control y nos permiten seleccionar cualquier cantidad de registros.

Las agregaciones recicladas también son útiles en conjunto con `mutate()`. Por ejemplo, con los datos de bateo, podríamos calcular el "año de carrera", el número de años que un jugador ha jugado desde que ingresó a la liga:
...
2. Encuentra los vuelos con un retraso mayor a una hora.
arr_delays_more_than_1hour
```{r, message=FALSE, warning=FALSE}
flights %>% 
  filter(arr_delay > 60) %>% 
  mutate(delay = arr_delay/60) %>% 
  arrange(desc(delay))
```

dep_delays_more_than_1hour
```{r, message=FALSE, warning=FALSE}
 flights %>% 
  filter(dep_delay > 60) %>% 
...
4. Ve la ayuda de select (`?select`) y escribe tres
maneras de seleccionar las variables de retraso (delay).
Creo que se resolvía de otra manera*
```{r, message=FALSE, warning=FALSE}
is_more_than_1hour <- function(delay) delay>60
retrasos <- flights %>% 
  select_all(is_more_than_1hour(arr_delay))
retrasos <- flights %>% 
  select_if(is_more_than_1hour(delay))
retrasos <- flights %>% 
  select_at(is_more_than_1hour)
```

5. ¿Cuál es la aerolínea con mayor retraso de salida por día de la semana? `departure_delay`
```{r, message=FALSE, warning=FALSE}
flights %>% 
  select(dep_delay, carrier, day) %>% 
  filter(dep_delay>0) %>% 
  group_by(day, carrier) %>% 
  summarise(max_delay = max(dep_delay)) %>% 
  filter(max_delay==max(max_delay)) %>% 
  left_join(nycflights13::airlines, c("carrier" = "carrier"))
```

6. ¿Qué aerolínea deberíamos tomar para tener la menor demora posible para llegar a Detroit? (ocupa `arrival_delay` como métrica, por qué ocupar esa variable?)
```{r, message=FALSE, warning=FALSE}
flights %>% 
  select(carrier, arr_delay, dest) %>% 
  filter(dest == "DTW") %>% 
  arrange(arr_delay) %>% 
  head(5)
```
...
max(velocidad$speed, na.rm = TRUE)
...
```{r, message=FALSE, warning=FALSE}
flights %>%
  select(flight, origin, dest, arr_delay, dep_delay) %>% 
  mutate(tiempo_extra = arr_delay-dep_delay) %>% 
  arrange(desc(tiempo_extra))
```
9. Calcula el retraso de llegada
 promedio por fecha.
```{r, message=FALSE, warning=FALSE}
flights %>% 
  filter(arr_delay > 0) %>% 
  mutate(dates = paste(year, "-", month, "-", day)) %>% 
  select(arr_delay, dates) %>% 
  group_by(dates) %>% 
  summarise(prom_retraso = mean(arr_delay)) %>% 
  arrange(desc(prom_retraso))
```
...
```{r, message=FALSE, warning=FALSE}
flights %>%
  filter(!is.na(air_time) & (dep_delay > 0 | arr_delay > 0)) %>% 
  mutate(dep_hora = hour + minute/60) %>%
  group_by(dep_hora) %>% 
  summarise(dep_delay_promedio = mean(dep_delay, na.rm = TRUE),
            arr_delay_promedio = mean(arr_delay, na.rm = TRUE))
```
...
flights %>%
  ...
  group_by(day_of_week, name) %>% 
  summarise(number_dep_delay = n()) %>% 
  group_by(name) %>% 
  filter(min_rank(number_dep_delay) == 1) %>% 
  arrange(name)
```

13. ¿Cuál es la 32va aerolínea con mayor diversidad de vuelos de entrada? Utiliza `rank`
```{r, message=FALSE, warning=FALSE}
nycflights13::airlines %>% count()
```
Sólo hay 16 aerolíneas, entonces el número de aeropuertos o carrieras tendría más sentido si se quisiera conocer un rank así, por lo que escogemos aeropuertos
```{r, message=FALSE, warning=FALSE}
flights %>% 
  group_by(dest) %>% 
  summarise(num_arr_flights = n()) %>% 
  filter(min_rank(num_arr_flights) == 32) %>% 
  left_join(nycflights13::airports, c("dest" = "faa"))
```
...
```{r, message=FALSE, warning=FALSE}
flights %>% 
  left_join(nycflights13::airports, c("origin" = "faa")) %>% 
...
```
-Puede significar que no se ejecutó el vuelo, porque no tienen datos de tiempo de vuelo con registro

Registros donde no hay coincidencia
```{r, message=FALSE, warning=FALSE}
flights %>% 
  left_join(nycflights13::planes, by = "tailnum") %>% 
  filter(!is.na(tailnum) & is.na(type))
```
...
```{r, message=FALSE, warning=FALSE}
flights %>% 
  select(flight, year, month, day, origin, dest, arr_delay, dep_delay) %>%
  group_by(year, month, day) %>% 
  summarise(mean_arr_delay = mean(arr_delay, na.rm = TRUE), mean_dep_delay = mean(dep_delay, na.rm = TRUE)) %>% 
  mutate(total_delay = mean_arr_delay + mean_dep_delay) %>% 
  mutate(delay_48_hours_1 = total_delay + (lag(total_delay))) %>%
  mutate(delay_48_hours_2 = total_delay + (lead(total_delay))) %>% 
  arrange(desc(delay_48_hours_1)) %>% 
  head(1) %>% 
...
flights %>%
  filter(arr_delay > 0) %>%
  group_by(time_hour) %>% summarise(sum = sum(arr_delay, na.rm = TRUE)) %>% 
  arrange(desc(sum)) %>% 
  head(48) %>% 
  left_join(nycflights13::weather, by = "time_hour") %>%
...
1.- Descarta lo que no haya coincidente entre flights y airports, de flights
2.- Descarta lo que no haya coincidente entre flights y airports, de airports
...
flights %>% 
  ...
  group_by(tailnum, carrier) %>%
  arrange(tailnum) %>% 
  mutate(is_diffeten_than_above = (tailnum == lag(tailnum))) %>% 
  filter(is_diffeten_than_above == TRUE)
```
Por lo que la hipótesis no se rechaza

Instalar paquetes para manejo de SQL
```{r}
...
```{r}
orders_tbl <- tbl(db_connect('Northwind'), "Orders")
```

Se verifica que los OrderID sean únicos
```{r}
orders_tbl %>% group_by(OrderID) %>% count() %>% filter(n > 1)
```

Si lo son, entonces podemos filtrar por OrderID
```{r}
orders_tbl %>% 
  group_by(OrderID) %>% 
  summarise(n = n()) %>% 
...
25. ¿De qué fechas tenemos datos? (año, mes y día mínimo y máximo)?
```{r, message=FALSE, warning=FALSE}
#Convirtiendo a data frame o tibble
orders_df <- orders_tbl %>% 
  arrange('OrderDate') %>% 
  collect()
...
orders_df %>% 
  ...
  arrange(OrderDate) %>% 
  mutate(dif_time = difftime(OrderDate, lag(OrderDate), units = "days")) %>% 
  group_by(dif_time) %>% 
  summarise(difference_days = n())
```

Se aprecia que hay diferencia de días en específico:
0, 1, 2.958, 3 y 3.04
...
dates %>%
  arrange(OrderDate) %>% 
  mutate(date_dif_days = difftime(OrderDate, lag(OrderDate), units = "days")) %>% 
  arrange(desc(date_dif_days)) %>% 
  head(1)
```
...
dates %>%
  arrange(OrderDate) %>% 
  mutate(date_dif_seconds = difftime(OrderDate, lag(OrderDate), units = "secs")) %>%
  arrange(date_dif_seconds) %>% 
  head(1)
```

29. ¿Cuál es el 7mo producto más vendido? Utiliza `rank` en tu respuesta
```{r, message=FALSE, warning=FALSE}
orders_details_df <- tbl(db_connect('Northwind'), "Order Details") %>% collect()
...
```{r, message=FALSE, warning=FALSE}
orders_dates <- orders_df %>% 
  left_join(orders_details_df, By = 'OrderID') %>%
  mutate(month_order = month(OrderDate)) %>% 
  group_by(month_order, ProductID) %>% 
  summarise(count = n()) %>% 
...
31. ¿Cuáles son jefes? Revisa el atributo `reportsto` indica su nombre `firstname`, apellido `lastname`, departamento al que pertenece `title`, desde cuándo está en la compañía `hiredate` y cuántos empleados tiene a su cargo
```{r, message=FALSE, warning=FALSE}
employees_df <- tbl(db_connect('Northwind'), "Employees") %>% collect()
```

```{r, message=FALSE, warning=FALSE}
employees_df %>% 
  filter(!is.na(ReportsTo)) %>% 
  group_by(ReportsTo) %>% 
  summarise(Accountability=n()) %>% 
  left_join(employees_df, c('ReportsTo' = 'EmployeeID'))
```


32. Por cada "jefe" muestra el nombre `firstname` y apellido `lastname` del jefe seguido de un arreglo de id empleados `employeeid` de los empleados que tiene a su cargo.

jefe1 <- tbl(northwind, 'Employees') %>%
  filter(ReportsTo != 'NA') %>% 
...
jefes_emp <- data.frame(EmployeeID = c(2,5), 
                        Employees = c(jefe1,jefe2)) %>% 
  inner_join(jefes) %>% 
  select(FirstName, 
         LastName, 
         Employees)
  ...
  # group_by(ReportsTo) %>% 
  # summarise(employees = as.list(unique(EmployeeID))) %>% 
  # View

33. ¿Cuál es el promedio de productos vendidos por orden y mes? 
promedio_productos_mes
```{r, message=FALSE, warning=FALSE}
orders %>% 
  left_join(orders_details_df, By = 'OrderID') %>% 
  mutate(month_nbr = month(OrderDate)) %>% 
  group_by(month_nbr, OrderID) %>% 
  summarise(count = n()) %>% 
  group_by(month_nbr) %>% 
  summarise(mean_month = mean(count))
```

promedio_productos_orden
```{r, message=FALSE, warning=FALSE}
 orders %>% 
  left_join(orders_details_df, By = 'OrderID') %>% 
  mutate(month_nbr = month(OrderDate)) %>% 
