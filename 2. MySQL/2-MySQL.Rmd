...
```{r, message=FALSE, warning=FALSE}
aux_flilght2 <- aux_flilght %>% 
  count(dest) %>% 
  arrange(desc(nn))
```

```{r, message=FALSE, warning=FALSE}
aiport_dest <- aux_flilght2 %>%
  inner_join(airports, c("dest" = "faa"))
```

2. Encuentra los vuelos con un retraso mayor a una hora.
```{r, message=FALSE, warning=FALSE}
arr_delays_more_than_1hour <- flights %>% 
  filter(arr_delay > 60) %>% 
  mutate(delay = arr_delay/60) %>% 
  arrange(desc(delay))

dep_delays_more_than_1hour <- flights %>% 
  filter(dep_delay > 60) %>% 
  mutate(delay = dep_delay/60) %>% 
  arrange(desc(delay))

delays_more_than_1hour <- flights %>% 
...
```{r, message=FALSE, warning=FALSE}
promedio_productos_orden <- orders %>% 
  left_join(orders_details_df, By = 'OrderID') %>% 
  mutate(month_nbr = month(OrderDate)) %>% 
  group_by(month_nbr, OrderID) %>% 
  summarise(count = n()) %>% 
