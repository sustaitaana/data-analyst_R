...
```{r}
# arrival_delay son los retrasos de llegada por lo que, la aerolinea que tenga menos retrasos de llegada es la que nos har?? llegar a Detroit con menor diferencia entre el retraso y la llegada estimada del vuelo (la demora).
...
7. Calcula la velocidad en millas por hora a partir de la variable tiempo y la distancia (en millas). ??Qu?? vuelo fue el m??s r??pido?
...
Primero notar que hay compa??ias que tienen m??s de un mismo vuelo en un mismo d??a
...
```{r}
flights %>% 
  left_join(airlines, by = "carrier") %>% #Solo para obtener el nombre de la compa????a
  ...
  dplyr::summarise(n()) %>% #Solo para obtener los datos agrupados en mes, d??a y compa??ia (no importa el conteo)
  group_by(day,company_flight) %>% #Con los anteriores datos se agrupa por d??a y ahora si importa el n??mero de registros
  dplyr::summarise(conteo = n()) %>% #Con esto se obtiene un conteo de cada n??mero de d??a por a??o(por ejemplo, cuantos d??as con el n??mero 1 existen (por obviedad, lo m??ximo es 12 )) 
  group_by(company_flight) %>% #Se agrupa por compa????a para ver cuantos d??as distintos existen 
  dplyr::summarise(total_days = sum(conteo)) %>% 
  arrange(desc(total_days)) %>% 
  filter(total_days>360) #Solo para obtener los vuelos que al menos 360 d??as en el a??o
```
...
Vease que solo existen 16 aerol??neas :

```{r}
airlines %>% count()
```

Por lo que se piensa que el ejercicio va dirigido no hacia las aerol??neas, si no a los aeropuertos (adem??s, no tendr??a alg??n sentido un vuelo de entrada ya que la misma aerol??nea esta involucrado en el despegue y aterrizaje )
...
Primero, con el promedio de retrasos por avi??n se tiene una medida de los retrasos.
...
Como hay aviones de los cuales no se tiene informaci??n del a??o, se toman solo aquellos donde se tenga la anterior informaci??n
...
Como se ve en la anterior gr??fica, existe una gran cantidad de aviones al rededor de los a??os 2000.

Nuevamente se toma el promedio por obtener otra medida de los retrasos, ahora por a??o del avi??n.
...
flights_grap_two <- flights %>% 
  ...
  ggtitle('Media de retrasos por a??o')
  
ggplotly(flights_grap_two)
  
```

La cantidad de datos de los aviones menores a 1984 son menores comparados a los posteriores a este a??o as??, hay aviones del a??o 2003 que tiene una mayor cantidad de retrasos que los aviones del a??o 2000, en lado contrario, hay aviones del a??o 2007 que tienen un mayor n??mero de retrasos que en el 2012, por lo que no hay alguna relaci??n con el tiempo de retraso y la longevidad del avi??n.
...
map_one <- flights %>% 
  ...
  coord_quickmap()+
  ggtitle('distribuci??n de retrasos promedio')
...
flights <- flights %>% 
  ...
  dplyr::select(-c(name, alt, tz, dst, tzone)) %>% 
  left_join(airports, c("dest" = "faa")) %>% 
...
```
Si no se tiene registro en el campo `tailnum` significa que no hubo un avi??n, es decir, no hubo vuelo.
...
Ahora, la mayor??a de los vuelos donde sucede lo anterior, la aerolinea encargada del vuelo es American Airways (AA) y Envoy Air (MQ) que en suma tienen m??s del 90% de estos vuelos.


Agregando a lo anterior, la documentaci??n dice lo siguiente : American Airways (AA) y Envoy Air (MQ) informan los n??meros de flota en lugar de los n??meros de cola, por lo que pueden no coincidir.
...
Primero, se buscar??n las 48 horas con mayor tiempo perdido
```{r}
flights %>% 
  group_by(year, month, day) %>% 
...
Ya teniendo estas referencias, se toma este d??a y el d??a anterior (por la utilizaci??n de lag)
...
weather_flights <- flights %>% 
  ...
  dplyr::summarise(temp_mean = mean(temp, na.rm = TRUE), wind_dir_mean = mean(wind_dir, na.rm = TRUE)) %>% # Ya que hay tres estaciones meteorol??gicas
  mutate(celcius = (temp_mean-32)/1.8000)
```

La temperatura se mantuvo en temperaturas bajas, no m??s de 5?? celcius

```{r}
weather_flights %>% ggplot(aes(x =time_hour, y = celcius)) + geom_point()
```

Y la direcci??n del viento (en grados), despu??s de un tiempo cercano al inicio, se mantuvo en un rango
...
weather_flights_2 <- flights %>% 
  ...
  left_join(nycflights13::weather) %>% 
  group_by(day, hour, time_hour) %>% 
  dplyr::summarise(temp_mean = mean(temp, na.rm = TRUE)) %>% # Ya que hay tres estaciones meteorol??gicas
  mutate(celcius = (temp_mean-32)/1.8000)
```
...
La temperatura se mantuvo en un rango de temperatura entre 23?? y 31?? centigrados
...
```
Los aeropuertos que no fueron considerados como destino de alg??n vuelo
...
flights %>% 
  ...
  filter(count > 1)
```
Por lo cu??l, es cierta la hip??tesis.
...
Como vemos, cada registro es ??nico, por lo que basta con contar el n??mero de registros
...
```
Se tienen datos cada 3, 1 o 0 d??as; como se tienen datos con diferencia de 3 d??as con el siguiente registro, los 2 registros faltantes (por los d??as) son las fechas faltantes
...
El tiempo delta m??s grande es de 3.041667 d??as.
