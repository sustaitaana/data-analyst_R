...
Agregando a lo anterior, la documentación dice lo siguiente : American Airways (AA) y Envoy Air (MQ) informan los números de flota en lugar de los números de cola, por lo que no pueden coincidir.
...
La demora se tomará mediante la variable `lost_time`
...
flights %>%
  filter(arr_delay > 0) %>%
  group_by(time_hour) %>% dplyr::summarise(sum = sum(arr_delay, na.rm = TRUE)) %>% 
  arrange(desc(sum)) %>% 
  head(48) %>% 
  left_join(weather, by = "time_hour") %>% #Por cada hora, se tienen tres registros en weather, uno por cada estación climatológica, por lo que se hace un promedio entre los tres valores resultantes.
  group_by(time_hour) %>% dplyr::summarise(mean_temp = mean(temp, na.rm = TRUE), mean_wind_dir = mean(wind_dir, na.rm = TRUE), mean_wind_speed = mean(wind_speed, na.rm = TRUE))
...
```{r}
orders_db %>% group_by(OrderID) %>% count() %>% filter(n > 1)
```

Como vemos, cada registro es único, por lo que basta con contar el número de registros

```{r}
orders_db %>% group_by(OrderID) %>% count() %>% group_by(n) %>% summarise(n())
```
...
```{r}
orders_arrange %>% mutate(Order_delta = difftime(OrderDate, lag(OrderDate), units = c( "days"))) %>% select(OrderDate, Order_delta)
```
...
31. ¿Cuáles son jefes? Revisa el atributo `reportsto` indica su nombre `firstname`, apellido `lastname`, departamento al que pertenece `title`, desde cu??ndo est?? en la compa????a `hiredate` y cu??ntos empleados tiene a su cargo
