...
Nos conectamos a la base de datos `northwind` que creamos en MySQL:
...
Notemos lo que pasa si imprimimos la tabla `employees`:

```{r, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
employees
# Source:   lazy query [?? x 18]
# Database: mysql 10.1.33-MariaDB [root@127.0.0.1:/northwind]
#   EmployeeID LastName  FirstName Title        TitleOfCourtesy
#        <int> <chr>     <chr>     <chr>        <chr>          
# 1          1 Davolio   Nancy     Sales Repre… Ms.            
# 2          2 Fuller    Andrew    Vice Presid… Dr.            
# 3          3 Leverling Janet     Sales Repre… Ms.            
# 4          4 Peacock   Margaret  Sales Repre… Mrs.           
# 5          5 Buchanan  Steven    Sales Manag… Mr.            
# 6          6 Suyama    Michael   Sales Repre… Mr.            
# 7          7 King      Robert    Sales Repre… Mr.            
# 8          8 Callahan  Laura     Inside Sale… Ms.            
# 9          9 Dodsworth Anne      Sales Repre… Ms.            
# ... with 13 more variables: BirthDate <chr>,
#   HireDate <chr>, Address <chr>, City <chr>, Region <chr>,
#   PostalCode <chr>, Country <chr>, HomePhone <chr>,
#   Extension <chr>, Notes <chr>, ReportsTo <int>,
#   PhotoPath <chr>, Salary <dbl>
```

Si queremos tener la tabla como un `data.frame` de R, debemos hacer un `collect` después del query que hicimos a la base de datos:
...
Si imprimimos, nuevamente, la tabla `employees`, notemos la diferencia:
