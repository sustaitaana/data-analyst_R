...
`dplyr` añade muchas funciones para manipulación de datos a través de data frames, con un gran performance y backends para varias bases de datos. Nos permite manipular directamente tablas de una BD, sin escribir SQL, toda la evaluaci??n es lazy y, de ser posible, se realiza directamente en la BD.
...
Una **funci??n de ventana** es una variaci??n de una funci??n de agregaci??n. Cuando una funci??n de agregaci??n, como `sum()` y `mean()`, toma $n$ entradas y devuelve un ??nico valor, una funci??n de ventana devuelve $n$ valores. La salida de una funci??n de ventana depende de todos sus valores de entrada, por lo que no incluyen funciones que sean utilizadas elemento a elemento, como `+` o `round()`. Las funciones de ventana incluyen variaciones en las funciones de agregaci??n, como `cumsum()` y `cummean()`, funciones para clasificar y ordenar como `rank()`, y funciones para encontrar contrapesos, como `lead()` y `lag()`.

Usaremos una peque??a muestra del conjunto de datos de bateo de `Lahman`, que contiene a los jugadores que han ganado un premio.
...
Las funciones de ventana regularmente se utilizan junto con `mutate()` y `filter()` para resolver una amplia gama de problemas. Por ejemplo:

+ Por cada jugador, encuentra los dos a??os con m??s hits

```{r, message=FALSE, warning=FALSE}
filter(players, min_rank(desc(H)) <= 2 & H > 0)
```

+ Para cada jugador, clasifica cada a??o por la cantidad de juegos jugados

```{r, message=FALSE, warning=FALSE}
mutate(players, G_rank = min_rank(G))
```

+ Para cada jugador, calcula el cambio promedio en los juegos jugados por a??o
...
Hay cinco familias principales de funciones de ventana. Dos de las cuales no est??n relacionadas con las funciones de agregaci??n:

+ Funciones de clasificaci??n y ordenamiento: `row_number()`, `min_rank()`, `dense_rank()`, `cume_dist()`, `percent_rank()`, y `ntile()`. Todas estas funciones toman un vector para ordenar y devuelven varios tipos de rangos.

+ Contrapesos `lead()` y `lag()` permiten acceder a los valores anteriores y siguientes en un vector, lo que facilita el c??lculo de diferencias y tendencias.

Las otras tres familias son variaciones en las funciones de agregaci??n que ya conocemos:

+ Agregaciones acumulativas: `cumsum()`, `cummin()`, `cummax()` (de la base R), y `cumall()`, `cumany()` y `cummean()` (de `dplyr`).

+ Las agregaciones m??viles operan en una ventana de ancho fijo. Estos no son parte de R base ni de `dplyr`, pero hay muchas implementaciones en otros paquetes, como [RcppRoll](https://cran.r-project.org/web/packages/RcppRoll/index.html).
  
+ Agregaciones recicladas, donde se repite una agregaci??n para que coincida con la longitud de la entrada. Estos no son necesarios en R porque el reciclaje de vectores recicla las agregaciones autom??ticamente cuando es necesario. Son importantes en SQL, porque la presencia de una funci??n de agregaci??n generalmente le dice a la base de datos que devuelva solo una fila por grupo.

#### rank

Las funciones de clasificaci??n son variaciones de un mismo tema, s??lo cambia c??mo se manejan las ligas a los datos:
...
Otras dos funciones de clasificaci??n devuelven n??meros entre 0 y 1. `percent_rank()` da el porcentaje del rango mientras que `cume_dist()` da la proporci??n de valores menores o iguales que el valor actual.
...
Estos son ??tiles si deseamos seleccionar (por ejemplo) el 10% superior de registros dentro de cada grupo. Por ejemplo:

```{r, message=FALSE, warning=FALSE}
filter(players, cume_dist(desc(G)) < 0.1)
```

Finalmente, `ntile()` divide los datos en $n$-cubos de tama??o uniforme. Es una clasificaci??n aproximada, y se puede utilizar `mutate()` para dividir los datos en segmentos para un resumen posterior. 

Por ejemplo, podr??amos usar `ntile()` para dividir a los jugadores dentro de un equipo en cuatro grupos clasificados y calcular el n??mero promedio de juegos dentro de cada grupo.
...
Todas las funciones de clasificaci??n se clasifican de menor a mayor, por lo que los valores de entrada peque??os obtienen rangos peque??os. Debemos usar `desc()` para clasificar de mayor a menor.

#### lead & lag

`lead()` y `lag()` producen versiones "movidas" de un vector de entrada que est?? por delante o por detr??s del vector original.
...
+ Calcular las diferencias o los cambios porcentuales.

```{r, message=FALSE, warning=FALSE}
mutate(players, G_delta = G - lag(G))
```

El uso `lag()` es m??s conveniente que `diff()` ya que para las $n$ entradas `diff()` devuelve $n - 1$ salidas.

+ Averiguar cu??ndo cambia un valor.

```{r, message=FALSE, warning=FALSE}
filter(players, teamID != lag(teamID))
```

`lead()` y `lag()` tienen un argumento opcional `order_by`. Si se establece, en lugar de usar el orden de fila para determinar qu?? valor viene antes que otro, usar??n otra variable. Esto es importante si no tenemos ordenados los datos, o si deseamos ordenarlos de una manera y retrasarlos.

Aqu?? un peque??o ejemplo de lo que sucede si no especificamos `order_by` cuando es necesario:
...
En R base tenemos algunas funciones acumultavias como la suma (`cumsum()`), el m??nimo (`cummin()`) y el m??ximo (`cummax()`). Otras funciones de acumulaci??n comunes son `cumany()` y `cumall()`, versiones acumulativas de `||` y `&&`, y `cummean()`, una media acumulativa. Estas pertenecen a `dplyr`.

`cumany()` y `cumall()` son ??tiles para seleccionar todas las filas hasta, o todas las filas posteriores a una condici??n que es verdadera la primera (o ??ltima) vez. Por ejemplo, podemos usar `cumany()` para encontrar todos los registros de un jugador despu??s de que haya jugado un a??o con 150 juegos:

```{r, message=FALSE, warning=FALSE}
filter(players, cumany(G > 150))
```

Al igual que con lead y lag, es posible que deseemos controlar el orden en que se produce la acumulaci??n. Ninguna de las funciones integradas tienen un argumento `order_by` argumento por lo que `dplyr` nos ofrece un ayudante: la funci??n `order_by()`:
...
Esta funci??n usa evaluaci??n no est??ndar, por lo que no es recomendable usarla dentro de otra funci??n, en su lugar se recomienda usar el m??s simple pero menos conciso `with_order()`.

#### Agregaciones recicladas

El reciclaje de vector de R facilita la selecci??n de valores que son m??s altos o m??s bajos que un resumen. Se llaman agregaciones recicladas porque el valor del agregado se recicla para que tenga la misma longitud que el vector original. Son ??tiles si estamos buscando todos los registros mayores que la media o menores que la mediana:
...
Si bien la mayor??a de las bases de datos SQL no tienen un equivalente de `median()` o `quantile()`, al filtrar podemos lograr el mismo efecto con `ntile()`. 

Por ejemplo, `x > median(x)` es equivalente a `ntile(x, 2) == 2`, `x > quantile(x, 75)` es equivalente a `ntile(x, 100) > 75` o `ntile(x, 4) > 3`.

```{r, message=FALSE, warning=FALSE}
filter(players, ntile(G, 2) == 2)
```

Tambi??n podemos usar esta idea para seleccionar los registros con el valor m??s alto (`x == max(x)`) o m??s bajo (`x == min(x)`) para un campo, pero las funciones de clasificaci??n nos brindan m??s control y nos permiten seleccionar cualquier cantidad de registros.

Las agregaciones recicladas tambi??n son ??tiles en conjunto con `mutate()`. Por ejemplo, con los datos de bateo, podr??amos calcular el "a??o de carrera", el n??mero de a??os que un jugador ha jugado desde que ingres?? a la liga:
...
35. Para cada jugador de la tabla `Lahman::Batting`, busca todos aquellos que hayan jugado m??s juegos que el promedio
...
```{r}
#Tercera forma
flights %>% select(-c(year, month, day, dep_time, sched_dep_time, arr_time, sched_arr_time, carrier, flight, tailnum, origin, dest, air_time, distance, hour, minute, time_hour
)) %>% head()
```
...
```{r}
departure <- flights %>%
  mutate(dow = weekdays(time_hour)) %>%
  group_by(dow) %>%
  dplyr::summarise(departure  = max(dep_delay, na.rm= TRUE))

fbd <- flights %>%
  mutate(dow = weekdays(time_hour)) %>%
  group_by(carrier, dow) %>%
  dplyr::summarise(departure  = max(dep_delay, na.rm= TRUE)) %>% 
  left_join(airlines, by = "carrier")

departure %>% left_join(fbd)
```
...
```{r}
# arrival_delay son los retrasos de llegada por lo que, la aerolinea que tenga menos retrasos de llegada es la que nos hará llegar a Detroit con menor diferencia entre el retraso y la llegada estimada del vuelo (la demora).

flights %>% 
  group_by(carrier) %>% 
  dplyr::summarise( delay_mean =  mean(arr_delay, na.rm = TRUE)) %>% 
  arrange(by = delay_mean) %>% 
  head(1) %>% 
  left_join(airlines, by = "carrier")

```

7. Calcula la velocidad en millas por hora a partir de la variable tiempo y la distancia (en millas). ¿Qué vuelo fue el más rápido?

```{r}
flights %>% 
  mutate(airTime_in_hours = air_time/60, velocity = distance/airTime_in_hours) %>% 
  select(flight, carrier, origin, dest, air_time, distance, airTime_in_hours, velocity) %>% 
  arrange(desc(by = velocity))
```

El vuelo más rapido es 1499

8. Crea una nueva variable que muestre cuánto tiempo se ganá o perdió durante el vuelo. ¿Qué vuelos _ganaron_ más tiempo en el aire?

```{r}
flights <- flights %>% mutate(loss_time = arr_delay+dep_delay)
flights %>% 
  arrange(loss_time) %>% 
  select(flight, origin, dest, loss_time)
```
...
flights%>%
  ...
  dplyr::summarise(loss_delay_mean = mean(loss_time, na.rm = TRUE))
```

10. ¿Qué vuelos (compañía + vuelo) ocurren diario?

Primero notar que hay compañias que tienen más de un mismo vuelo en un mismo día

```{r}
flights %>% 
  group_by(flight, day, month, carrier) %>% 
  dplyr::summarise(flights_by_day = n()) %>% 
  filter(flights_by_day >1)
```
...
```{r}
flights %>% 
  left_join(airlines, by = "carrier") %>% #Solo para obtener el nombre de la compañía
  mutate (company_flight = paste(name, as.character(flight), sep = "-")) %>% #Se agrega columna para identificar vuelo
  group_by(month,day, company_flight) %>% 
  dplyr::summarise(n()) %>% #Solo para obtener los datos agrupados en mes, día y compañia (no importa el conteo)
  group_by(day,company_flight) %>% #Con los anteriores datos se agrupa por día y ahora si importa el número de registros
  dplyr::summarise(conteo = n()) %>% #Con esto se obtiene un conteo de cada número de día por año(por ejemplo, cuantos días con el número 1 existen (por obviedad, lo máximo es 12 )) 
  group_by(company_flight) %>% #Se agrupa por compañía para ver cuantos días distintos existen 
  dplyr::summarise(total_days = sum(conteo)) %>% 
  arrange(desc(total_days)) %>% 
  filter(total_days>360) #Solo para obtener los vuelos que al menos 360 días en el año
```

11. En promedio, ¿Cómo varían a lo largo del día los retrasos de vuelos no cancelados? (pista: hour + minute / 60)

Los vuelos cancelados son aquellos que no tienen tiempo en el aire
```{r}
flights %>% 
  filter(!is.na(air_time) & dep_delay > 0) %>% 
  mutate(departure_hours = hour + minute/60) %>%
  group_by(departure_hours) %>% 
  dplyr::summarise(dep_mean_minutes = mean(dep_delay, na.rm = TRUE))
...
flights %>%
  ...
  left_join(airlines, by = "carrier") %>% 
  mutate(dow = weekdays(time_hour)) %>% 
  group_by(dow, name) %>% dplyr::summarise(number_dep_delay = n()) %>% 
  group_by(name) %>% filter(min_rank(number_dep_delay) == 1) %>% 
  arrange(name)
```

13. ¿Cuál es la 32va aerolínea con mayor diversidad de vuelos de entrada? Utiliza `rank`

Vease que solo existen 16 aerolíneas :

```{r}
airlines %>% count()
```

Por lo que se piensa que el ejercicio va dirigido no hacia las aerolíneas, si no a los aeropuertos (además, no tendría algún sentido un vuelo de entrada ya que la misma aerolínea esta involucrado en el despegue y aterrizaje )

```{r}
# Vuelos de entrada : dest
flights %>% 
  group_by(dest) %>% 
  dplyr::summarise(number_arr_flights = n()) %>% 
  filter(min_rank(number_arr_flights) == 32) %>% 
  left_join(airports, c("dest" = "faa"))
```

14. Explora si los aviones más viejos están asociados a mayores retrasos, responde con una gráfica.

Primero, con el promedio de retrasos por avión se tiene una medida de los retrasos.

```{r}
flights %>% 
  filter(dep_delay >0 ) %>% 
  group_by(tailnum) %>% 
  dplyr::summarise(mean_of_delay_plane = mean(dep_delay, na.rm = TRUE)) %>% 
  left_join(planes, by = 'tailnum')
```

Como hay aviones de los cuales no se tiene información del año, se toman solo aquellos donde se tenga la anterior información

```{r}
library(plotly)
flights_grap_one <- flights %>% 
  filter(dep_delay >0 ) %>% 
  group_by(tailnum) %>% 
  dplyr::summarise(mean_of_delay_plane = mean(dep_delay, na.rm = TRUE)) %>% 
  left_join(planes, by = 'tailnum') %>% 
  filter(!is.na(year)) %>% select(tailnum, mean_of_delay_plane, year) %>% 
  ggplot(aes(x = year, y = mean_of_delay_plane))+ 
  geom_jitter()+
  aes(colour=mean_of_delay_plane)+
  ggtitle('media de retrasos por avion')
ggplotly(flights_grap_one)
```

Como se ve en la anterior gráfica, existe una gran cantidad de aviones al rededor de los años 2000.

Nuevamente se toma el promedio por obtener otra medida de los retrasos, ahora por año del avión.

```{r}
flights_grap_two <- flights %>% 
  filter(dep_delay >0 ) %>% 
  group_by(tailnum) %>% 
  dplyr::summarise(mean_of_delay_plane = mean(dep_delay, na.rm = TRUE)) %>% 
  left_join(planes, by = 'tailnum') %>% 
  filter(!is.na(year)) %>% select(tailnum, mean_of_delay_plane, year) %>% 
  group_by(year) %>% dplyr::summarise(mean_of_delay = mean(mean_of_delay_plane, na.rm = TRUE)) %>% 
  ggplot(aes(x = year, y = mean_of_delay, fill = mean_of_delay))+ 
  geom_bar(stat = 'identity')+
  ggtitle('Media de retrasos por año')
  
ggplotly(flights_grap_two)
  
```

La cantidad de datos de los aviones menores a 1984 son menores comparados a los posteriores a este año, aún así, hay aviones del año 2003 que tiene una mayor cantidad de retrasos que los aviones del año 2000, en lado contrario, hay aviones del año 2007 que tienen un mayor número de retrasos que en el 2012, por lo que no hay alguna relación con el tiempo de retraso y la longevidad del avión.
...
map_one <- flights %>% 
  filter(arr_delay > 0) %>% 
  group_by(dest) %>% dplyr::summarise(mean_delay = mean(arr_delay, na.rm = TRUE)) %>%  
  left_join(airports, c("dest" = "faa")) %>% 
  filter(!is.na(lat)) %>% 
  ggplot(aes(lon, lat)) +
  borders("state", colour="black") +
  geom_point(aes(col=mean_delay),size=2) +
  coord_quickmap()+
  ggtitle('distribución de retrasos promedio')

ggplotly(map_one)
```

Para ver mejor el mapa, se eliminan los dos datos que salen de la zona delimitada
...
map_two <- flights %>% 
  ...
  left_join(airports, c("dest" = "faa")) %>% 
  filter(!is.na(lat)) %>% 
  filter(!(mean_delay > 34.5 & mean_delay < 34.9) & !(mean_delay == 12.4), !is.na(lat)) %>%
  ggplot(aes(lon, lat)) +
  borders("state", colour="black") +
  geom_point(aes(col=mean_delay),size=3) +
  coord_quickmap()+
  ggtitle('distribución de retrasos promedio')
...
flights <- flights %>% 
  ...
  select(-c(name, alt, tz, dst, tzone)) %>% 
  left_join(airports, c("dest" = "faa")) %>% 
...
```{r}
flights %>% filter(year == 2013 & month == 6, day == 13) %>% View
```
...
```{r}
flights %>% filter(is.na(tailnum)) %>% head()
```
Si no se tiene registro en el campo `tailnum` significa que no hubo un avión, es decir, no hubo vuelo.

Registros donde no hay coincidencia:

```{r}
tail_but_no_plane <- flights %>% left_join(planes, by = "tailnum") %>% filter(!is.na(tailnum) & is.na(type))
tail_but_no_plane
```

Ahora, la mayoría de los vuelos donde sucede lo anterior, la aerolinea encargada del vuelo es American Airways (AA) y Envoy Air (MQ) que en suma tienen más del 90% de estos vuelos.


Agregando a lo anterior, la documentación dice lo siguiente : American Airways (AA) y Envoy Air (MQ) informan los números de flota en lugar de los números de cola, por lo que pueden no coincidir.

```{r}
tail_but_no_plane %>% 
  group_by(carrier) %>% 
  dplyr::summarise(conteo = n()) %>% mutate(percent = conteo/sum(conteo)) %>% 
...
Primero, se buscarán las 48 horas con mayor tiempo perdido
```{r}
flights %>% 
  group_by(year, month, day) %>% 
  summarise(mean_delay = mean(loss_time, na.rm = TRUE)) %>% 
  mutate(delay_48_hours = mean_delay + (lag(mean_delay))) %>% 
  arrange(desc(delay_48_hours)) %>% 
  head(1)
```
...
Ya teniendo estas referencias, se toma este día y el día anterior (por la utilización de lag)

```{r message=FALSE, warning=FALSE}
weather_flights <- flights %>% 
  select(year, month, day, time_hour) %>% 
  filter(year == 2013, month == 3, day %in% c(7, 8)) %>% 
  group_by(year, month, day, time_hour) %>% 
  dplyr::summarise(n()) %>%
  select(year, month, day, time_hour) %>%
  left_join(nycflights13::weather) %>% 
  group_by(day, hour, time_hour) %>% 
  dplyr::summarise(temp_mean = mean(temp, na.rm = TRUE), wind_dir_mean = mean(wind_dir, na.rm = TRUE)) %>% # Ya que hay tres estaciones meteorológicas
  mutate(celcius = (temp_mean-32)/1.8000)
```

La temperatura se mantuvo en temperaturas bajas, no más de 5º celcius

```{r}
weather_flights %>% ggplot(aes(x =time_hour, y = celcius)) + geom_point()
```

Y la dirección del viento (en grados), después de un tiempo cercano al inicio, se mantuvo en un rango

```{r}
weather_flights %>% ggplot(aes(x = time_hour, y = wind_dir_mean)) + geom_line()
```

Now, with the arrival delay
...
```{r message=FALSE, warning=FALSE}
weather_flights_2 <- flights %>% 
  select(year, month, day, time_hour) %>% 
  filter(year == 2013, month == 7, day %in% c(22, 23)) %>% 
  group_by(year, month, day, time_hour) %>% 
  dplyr::summarise(n()) %>%
  select(year, month, day, time_hour) %>%
  left_join(nycflights13::weather) %>% 
  group_by(day, hour, time_hour) %>% 
  dplyr::summarise(temp_mean = mean(temp, na.rm = TRUE)) %>% # Ya que hay tres estaciones meteorológicas
  mutate(celcius = (temp_mean-32)/1.8000)
```


```{r}
weather_flights_2 %>% ggplot(aes(x =time_hour, y = celcius)) + geom_line()
```

La temperatura se mantuvo en un rango de temperatura entre 23º y 31º centigrados
...
```
Los vuelos que tuvieron un destino diferente a cualquiera que estuviera en `airports`

```{r}
anti_join(airports, flights, by = c("faa" = "dest"))
```
Los aeropuertos que no fueron considerados como destino de algún vuelo
...
```{r}
flights %>% 
  group_by(tailnum, carrier) %>% #Deseamos los datos agrupados, no importa el summarise
  dplyr::summarise(n()) %>% 
  group_by(tailnum, carrier) %>% #Ya agrupados, si importa la cantidad de registros
  dplyr::summarise(count = n()) %>% 
  filter(count > 1)
```
Por lo cuál, es cierta la hipótesis.
...
```{r message=FALSE, warning=FALSE}
orders_db <- tbl(db_connect('Northwind'), "Orders")
```
...
Como vemos, cada registro es único, por lo que basta con contar el número de registros
...
Entonces hay 830 registros diferentes.

25. ¿De qué fechas tenemos datos? (año, mes y día mínimo y máximo)?
```{r message=FALSE, warning=FALSE}
orders_arrange <- orders_db %>% arrange('OrderDate') %>% collect()
```

Se tienen datos desde: 

```{r echo=FALSE}
orders_arrange$OrderDate %>% head(1)
```

Hasta:

```{r echo=FALSE}
orders_arrange$OrderDate %>% tail(1)
```
...
```{r}
orders_arrange %>% 
  mutate(Order_delta = difftime(OrderDate, lag(OrderDate), units = c( "days"))) %>% 
  select(OrderDate, Order_delta)
```
Se tienen datos cada 3, 1 o 0 días; como se tienen datos con diferencia de 3 días con el siguiente registro, los 2 registros faltantes (por los días) son las fechas faltantes
...
orders_arrange %>% 
  mutate(Order_delta = difftime(OrderDate, lag(OrderDate), units = c( "days"))) %>% 
  select(OrderDate, Order_delta) %>% 
  filter(Order_delta > 3)
```

El tiempo delta más grande es de 3.041667 días.
...
```{r}
orders_arrange %>% 
  mutate(Order_delta = difftime(OrderDate, lag(OrderDate), units = c( "secs"))) %>% 
  select(OrderDate, Order_delta) %>% 
  filter(Order_delta <= 1)
```

El tiempo más pequeño es de 0.

29. ¿Cuál es el 7mo producto más vendido? Utiliza `rank` en tu respuesta

```{r message=FALSE, warning=FALSE}
Ordern_details_db <- tbl(db_connect('Northwind'), "Order Details") %>% collect()
```
...
```{r}
orders_arrange %>% 
  left_join(Ordern_details_db, By = 'OrderID') %>%
  mutate(Month_of_purchase = lubridate::month(OrderDate)) %>% 
  group_by(Month_of_purchase, ProductID) %>% 
  dplyr::summarise(count = n()) %>% 
...
```{r}
id_jefes <- tbl(db_connect('Northwind'), 'Employees') %>% 
  select(ReportsTo) %>% 
  filter(ReportsTo != 'NA') %>% 
  collect() %>% 
  unique

jefes <- tbl(db_connect('Northwind'), 'Employees') %>% 
  filter(EmployeeID %in% c(2,5)) %>% 
  select(EmployeeID, FirstName, LastName, Title, HireDate) %>% 
  collect()

count <- tbl(db_connect('Northwind'), 'Employees') %>% 
  select(EmployeeID,ReportsTo, FirstName, LastName, Title, HireDate) %>% 
  collect() %>% group_by(ReportsTo) %>% dplyr::summarise(Number_employees = n()) %>% filter(!is.na(ReportsTo))
```
...
```{r}
jefes %>% 
  left_join(count, c("EmployeeID" = "ReportsTo")) %>% 
  select(-EmployeeID)
```
...
```{r}
jefe_1 <- tbl(db_connect('Northwind'), 'Employees') %>% 
  filter(ReportsTo != "NA") %>% 
...
jefe_2 <- tbl(db_connect('Northwind'), 'Employees') %>% 
  filter(ReportsTo != "NA") %>% 
  select(EmployeeID, ReportsTo) %>% 
  collect() %>% 
  filter(ReportsTo == 5) %>% 
...
Jefes_empleados <- data_frame(EmployeeID = c(2,5), Employees = c(jefe_1, jefe_2)) %>% inner_join(jefes, by = "EmployeeID") %>% select(FirstName, LastName, Employees)
...
orders_arrange %>% 
  left_join(Ordern_details_db, By = 'OrderID') %>%
  mutate(Month_of_purchase = lubridate::month(OrderDate)) %>% 
  group_by(Month_of_purchase, OrderID) %>% 
...
orders_arrange %>%
  ...
  dplyr::summarise(count = n()) %>%
  mutate(rank = min_rank(desc(count))) %>% 
  filter(ShipCity == 'Mxico D.F.') %>% 
  select(ShipCity, rank)
```

35. Para cada jugador de la tabla `Lahman::Batting`, busca todos aquellos que hayan jugado más juegos que el promedio

```{r}
library(Lahman)
mean <- mean(Batting$G)
Batting %>% filter(G > mean) %>% select(playerID, G)
```
